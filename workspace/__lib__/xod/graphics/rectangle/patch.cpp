
// clang-format off
{{#global}}
// clang-format on

#include <Rectangle.h>

// clang-format off
{{/global}}
// clang-format on

struct State {
    uint8_t mem[sizeof(Rectangle)];
    Rectangle* rectangle;
    int16_t x, y, w, h;
};

// clang-format off
{{ GENERATED_CODE }}
// clang-format on

void evaluate(Context ctx) {
    auto state = getState(ctx);

    auto gfx = getValue<input_GFX>(ctx);

    int16_t x = (int16_t)getValue<input_X>(ctx);
    int16_t y = (int16_t)getValue<input_Y>(ctx);
    int16_t w = (int16_t)getValue<input_W>(ctx);
    int16_t h = (int16_t)getValue<input_H>(ctx);

    if (isSettingUp()) {
        state->rectangle = new (state->mem) Rectangle(gfx, x, y, w, h);
        emitValue<output_GFXU0027>(ctx, state->rectangle);
    }

    if (x != state->x || y != state->y || w != state->w || h != state->h) {
        state->x = x;
        state->y = y;
        state->w = w;
        state->h = h;
        state->rectangle->setParams(x, y, w, h);
        emitValue<output_GFXU0027>(ctx, state->rectangle);
    }

    if (isInputDirty<input_GFX>(ctx)) {
        emitValue<output_GFXU0027>(ctx, state->rectangle);
    }
}
